...
Tabs = {
   ...
   Exploits       = Window:AddTab('  功能'),
   Visuals       = Window:AddTab('  视觉'),
   Floor     = Window:AddTab('  楼层'),
   Fun     = Window:AddTab('  娱乐'),
   UISettings = Window:AddTab("  设置"),
   Addons = Window:AddTab('  插件'),
 
}

local SupportSignal = ExecSupport["firesignal"]
local SpawnEntities = Tabs.Fun:AddRightGroupbox('生成客户端实体')
local GetItems = Tabs.Fun:AddLeftGroupbox('十字架')
_G.Uses = 1
_G.Range = 30
_G.OnAnything = true
_G.Fail = false
GetItems:AddLabel('十字架 由 PenguinManiack 制作',true)
GetItems:AddLabel('按 Q 激活十字架',true)
...
local Movement = Tabs.Player:AddLeftGroupbox('玩家')
...
local Anti = Tabs.Exploits:AddLeftGroupbox("防实体")

local Troll = Tabs.Exploits:AddLeftGroupbox("FE")
...
Notify = Tabs.Exploits:AddRightGroupbox("氧气")
...
local  Bypass = Tabs.Exploits:AddRightGroupbox("绕过")


Automation = Tabs.Player:AddRightGroupbox('自动')



local ReachBox = Tabs.Player:AddRightGroupbox('互动')
...
ModifiersBox = Tabs.Floor:AddLeftGroupbox('游戏')
...
ESP = Tabs.Visuals:AddRightGroupbox('ESP 显示')
Camera = Tabs.Visuals:AddRightGroupbox('相机')
Ambient = Tabs.Visuals:AddRightGroupbox('视觉')
SettingsESP = Tabs.Visuals:AddLeftGroupbox('ESP 设置')
ClosetTran = Tabs.Visuals:AddLeftGroupbox('透明设置')

local Buttons = Tabs.Player:AddLeftGroupbox('功能')

local MenuGroup = Tabs["UISettings"]:AddLeftGroupbox("菜单设置", "wrench")
...
FloorAnti:AddLabel('速度建议45-50，别开穿墙且挂好加速器',true)
...
Library:Notify("你的注入器不支持该功能",3)
return 
end

if queue_on_teleport then

Library:Notify("正在刷取 请等待ing",2)
...
InfiniteBox:AddToggle('InfiniteCrucifix', {
	...
	Default = false,
    Risky = true,
    Tooltip = "存在风险，可能导致死亡或丢失十字架，建议在低延迟（低ping）和稳定帧率（高帧率更佳）情况下使用"
})
...
Library:Notify("重置已停止",4)

end
if Continue then
Library:Notify("重置中请稍候，再次双击可停止",4)
end
repeat task.wait() 
...
Automation:AddDropdown("IgnoreList", {
        Values = {"杰夫商店的物品","金币","掉落物","错误方块","死亡糖果"},
        Default = 1, -- number index of the value / string
        Multi = true, -- true / false, allows multiple choices to be selected
...
if Value then
Library:Notify("自动获取错误方块已开启",4)
else
Library:Notify("自动获取错误方块已停止",4)
end
end
})
...
Sound()
Library:Notify("已生成一个错误方块 在箱子里或角落里..",5)
end
...
Automation:AddLabel('缺失数字的书本', true)
...
Automation:AddToggle('AutoCloset',{
     ...
     Default = false
		...
		Text = '自动柜子', -- Text to display in the keybind menu
		NoUI = false, -- Set to true if you want to hide from the Keybind menu,
...
Library:Notify("速度绕过方法2无法工作，因为您的执注入器不支持Sethiddenproperty",5)
...
Options.FlySpeed:SetMax(Value and 150 or 21)
...
Library:Notify("如果你选择了自动化无敌模式 反飞哥听力可能需要开无敌才能生效",3)
end
...
Bypass:AddToggle('AnticheatManipulation',{
     ...
     Default = false
		...
		Text = '穿墙不拉回', -- Text to display in the keybind menu
		NoUI = false, -- Set to true if you want to hide from the Keybind menu,
...
if foundAnchor then
                ...
                if not noteObj then
                    Library:Notify(("Anchor %s 密码是 %s"):format(foundAnchor.Sign.TextLabel.Text, currentCode), 3)
                else
                    ...
                    Library:Notify(("Anchor %s 密码是 %s"):format(foundAnchor.Sign.TextLabel.Text, newcode), 1)
                end
            end
        end
end
...
Library:Notify("现在你仅需要找到梯子并爬上（｡ò ∀ ó｡）",9)
...
Library:Notify("绕过反作弊会在 Halt房间和Seek追逐战 失效",7)
end
...
AddESP(Door, (Door.Parent.Parent:GetAttribute("RequiresKey") and "[上锁的]  " or "") .. "门 " .. Door.Parent:GetAttribute("RoomID"), DoorColor)
AddESP(Door2, (Door2.Parent.Parent:GetAttribute("RequiresKey") and "[上锁的]  " or "") .. "门 " .. Door2.Parent:GetAttribute("RoomID"), DoorColor)
...
ESPLibrary:RemoveESP(v.Door)
	...
	Title = '门颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '计时拉杆颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '拉杆颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '发电机颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '保险丝颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '锚点颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(Key)
	...
	Title = '钥匙颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '躲藏点颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(Lever)
	...
	Title = '门闸', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(plr.Character)
	...
	Title = '玩家颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '书颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '开锁器颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(i)
	...
	Title = '物品颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
ESPLibrary:RemoveESP(v)
	...
	Title = '金币颜色', -- Optional. Allows you to have a custom color picker title (when you open it)
	Transparency = 0, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
...
if Options.GMDropdown and Options.GMDropdown.Value == "全自动" then
...
if GUI then
GUI:FindFirstChild("OxygenText").Text = "当前氧气: "  ..  "%" ..  math.floor(LocalPlayer.Character:GetAttribute("Oxygen"))
...
if prompt.Parent and prompt.Parent.Name == "GoldPile" and Options.IgnoreList.Value["金币"] then continue end
if prompt.Parent:GetAttribute("JeffShop")  and Options.IgnoreList.Value["杰夫商店的物品"] then continue end
if prompt.Parent.Parent and prompt.Parent.Parent.Name == "Drops" and Options.IgnoreList.Value["掉落物"] then continue end
...
if prompt.Parent and prompt.Parent.Name == "GlitchCube" and Options.IgnoreList.Value["错误方块"] then continue end
if prompt.Parent and prompt.Parent.Name == "GlitchCube" and Options.IgnoreList.Value["Glitch Fragment"] then continue end
if prompt.Parent and prompt.Parent:GetAttribute("Tool_CandyID") == "CandyRed" and Options.IgnoreList.Value["死亡糖果"] then
 continue end
...
AddESP(Door, (Door.Parent:FindFirstChild("Lock") and "[上锁的]  " or "") .. "门 " .. Door.Parent:GetAttribute("RoomID"), DoorColor)
AddESP(Door2, (Door2.Parent.Parent:GetAttribute("RequiresKey") and "[上锁的]  " or "") .. "门 " .. Door2.Parent:GetAttribute("RoomID"), DoorColor)
...
if Lever then 
AddESP(Lever,"拉杆 ",GateLeverColor)
...
MenuGroup:AddLabel("菜单快捷键"):AddKeyPicker("菜单快捷键", { Default = "RightShift", NoUI = true, Text = "菜单快捷键" })
Library.ToggleKeybind = Options.MenuKeybind -- Allows you to have a custom keybind for the menu

	local Contributors = Tabs.UISettings:AddRightGroupbox("Credits")
	Contributors:AddLabel("KardinCat - 创作者",true)
Contributors:AddLabel("rhyan57 - API 支持脚本的创作者",true)
Contributors:AddButton('加入 msdoors 的 Discord',function()
if toclipboard then
...
MenuGroup:AddLabel('数值越低，开门时冻结时间越长，但处理速度越快', true)
...
Library:Notify("已复制 Discord 链接到剪贴板",3)
